...
describe("controller: BooksController", function() {
  var scope, ctrl, $httpBackend, myBookService;
  ...
  beforeEach(inject(function(_$httpBackend_, $rootScope, $controller, BookService) {

    $httpBackend = _$httpBackend_;

    $httpBackend.expectGET('api/books').
        respond([{title: 'Great Expectations', author: 'Dickens'},
                  {title: 'Foundation Series', author: 'Asimov'},
                  {title: 'Coming From Different Node', author: 'RobZ'},
                  {title: 'Treasure Island', author: 'Stephenson'}]);

    $httpBackend.expectGET('api/version').
        respond({version: 'Unit'});

    myBookService = BookService;
    spyOn(BookService, 'getBooks').andCallThrough();

    scope = $rootScope.$new();
    ctrl = $controller('BooksController', {$scope: scope});
  }));

  it('should call the book service to load books', function() {
    expect(myBookService.getBooks).toHaveBeenCalled();
  });

  it("should list 4 books", function() {
    expect(scope.books).toBeUndefined();
    $httpBackend.flush();
   
    expect(scope.books).toEqual([{title: 'Great Expectations', author: 'Dickens'},
                    {title: 'Foundation Series', author: 'Asimov'},
                    {title: 'Coming From Different Node', author: 'RobZ'},
                    {title: 'Treasure Island', author: 'Stephenson'}]);
...
describe("controller: MemberProfileController", function() {
    var scope, ctrl, $httpBackened;
    var rscope;
    ...
    beforeEach(inject(function(_$httpBackend_, $rootScope, $controller) {
        ...
        $httpBackend.expectGET('/api/v1/members/abcde').
            respond({
                id: 'abcde',
                firstName: 'Francisco',
                lastName: 'MacDonald',
        ...
        rscope = $rootScope;
        scope = $rootScope.$new();
        ctrl = $controller('MemberProfileController', {$scope: scope});
        $httpBackend.flush();
    }));

    it('should load Francisco as a member when his ID is requested', function() {
        expect(scope.member).toEqual({
            id: 'abcde',
            firstName: 'Francisco',
            lastName: 'MacDonald',
            title: 'President',
    ...
    it('should save when the save() function is called and redirect to view with a message', function() {
        scope.member.firstName = 'Francisca';
        var toSend = {
            id: 'abcde',
            firstName: 'Francisca',
            lastName: 'MacDonald',
        ...
        $httpBackend.expectPOST('/api/v1/members/abcde', toSend).
            respond(200, toSend);
        scope.save();
        $httpBackend.flush();
        expect(rscope.alerts[0].type).toEqual("alert-success");
    });
    it('should show errors if the save fails for some reason', function() {
        $httpBackend.expectPOST('/api/v1/members/abcde').
            respond(500, {});
        scope.save();
        $httpBackend.flush();
        expect(rscope.alerts[0].type).toEqual("alert-warning");
    });
});
...
describe("directive: shows-message-when-hovered (vanilla jasmine, coffeescript)", function() {
  ...
  describe("service: CompanyService", function() {
    var $httpBackend, myCompanyService;

    beforeEach(function() {
      module("app");
    });

    beforeEach(inject(function(_$httpBackend_, $rootScope, $controller, CompanyService) {
      $httpBackend = _$httpBackend_;
      myCompanyService = CompanyService;
    }));

    describe('the CompanyService module', function() {

      describe('#get', function() {
        it('returns data for a specified company ID', function() {
          $httpBackend.expectGET('/api/companies/asdf').
              respond(
                { id: 'asdf', 
                  name: 'All Pro Pizza',
                  address: '1615 S. Mooney Blvd',
                  city: 'Visalia',
                  stateAbbrv: 'CA',
                  zip: '93277',
                  phone: '5597388282',
                  shortDescription: 'Family owned pizza restaurant. Dine in, take out, and delivery',
                  longDescription: 'Parties, catering, sports, salad bar, delivery',
                  numEmployees: 100
                });

          myCompanyService.get('asdf').then(function(stuff) {
      ...
      describe('#save', function() {
        it('returns success when creating a new company', function() {
          $httpBackend.expectPOST('/api/companies', 
                { id: '1234', 
                  ...
                  zip: '43210',
                  phone: '5729871234',
                  shortDescription: 'We sell stuff',
                  longDescription: 'We sell lots of stuff'
                }).respond(200);

          myCompanyService.save( 
                { id: '1234', 
                  name: 'Some New Company',
                  address: '243 E. Main St.',
                  city: 'Columbus',
    ...
    describe('Test spy', function() {
      ...
      beforeEach(function() {
        ...
        // spy on the someFunction method of someObject
        spyOn(someObject, 'someFunction').andCallThrough();

        console.log(someObject.someFunction("Rob"));
        //asdf
      });

      it('can tell when the spied upon thing has been called', function() {
